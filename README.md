...
- **规则解析兼容性增强**
  - 修复 `IP-CIDR`/`IP-CIDR6` 与实际网段版本不一致问题（自动纠正）
  - 新增对“裸域名列表”格式支持（如 `china_domain_list.txt`），自动映射为 `DOMAIN-SUFFIX`
  - 支持单标签后缀（如 `cn`、`xn--fiqs8s`），避免产出 `OTHER,*` 非 Clash 标准规则

- **跨文件去重与优先级归属**
  - 新增跨文件优先级去重模块：`app/cross_dedupe.py`
  - 支持 `Coke.list` 人工覆盖优先级（最高）
  - 按优先级保留规则归属，低优先级文件自动剔除重复规则

- **构建稳定性与性能优化**
  - 抓取层改为并发抓取（保序）+ 指数退避重试
  - 当某任务来源全部抓取失败时，保留历史产物，避免空文件覆盖

- **统计口径修正**
  - `tx.log` 统计范围改为仅统计 `rule.json` 中定义的任务输出，避免历史残留文件干扰
